select ACCOUNT.ITSNAME as ITSNAME,
...
from
  (
    select ACC, SUBACCID, SUBACCT, sum(DEBIT) as DEBIT, sum(CREDIT) as CREDIT
    from 
      (
        select ITSACCOUNT as ACC, SUBACCID, BALANCEAT.SUBACCTYPE as SUBACCT,
        ...
        select ACCDEBIT as ACC, SUBACCDEBITID as SUBACCID, SUBACCDEBITTYPE as SUBACCT, sum(DEBIT) as DEBIT, 0.00 as CREDIT
        from  ACCOUNTINGENTRY 
        where ACCOUNTINGENTRY.ACCDEBIT=:ACCID :SUBACCDEBIT and ITSDATE>=:DATEBALANCE and ITSDATE<=:DATE1
        group by ACC, SUBACCID, SUBACCT
        
        union all
        
        select ACCCREDIT as ACC, SUBACCCREDITID as SUBACCID, SUBACCCREDITTYPE as SUBACCT, 0 as DEBIT, sum(CREDIT) as CREDIT
        from  ACCOUNTINGENTRY 
