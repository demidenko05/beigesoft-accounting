...
public class PrcSalesReturnLineSave<RS>
  ...
  public final void adjustInvoiceLns(final Map<String, Object> pReqVars,
    ...
    List<SalesReturnLine> igls = getSrvOrm().retrieveListWithConditions(
        pReqVars, SalesReturnLine.class,
          "where SALESRETURNLINE.TAXCATEGORY is not null"
            + " and REVERSEDID is null and ITSOWNER=" + pItsOwner.getItsId());
    pReqVars.remove("SalesReturnLineitsOwnerdeepLevel");
    List<SalesReturnLine> ilnt = new ArrayList<SalesReturnLine>();
    Comparator<SalesReturnLine> cmpr = Collections
      .reverseOrder(new CmprSalRetLnTotal());
